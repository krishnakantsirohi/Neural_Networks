...
class Hebbian(object):
    ...
    def train(self, X, y, batch_size=1, num_epochs=10, alpha=0.1, gamma=0.9, learning="Delta"):
        ...
        """
        X_with_1 = np.insert(X, 0, 1, axis=0)
        X_without_1 = np.array(X)
        Y_binary = np.zeros(shape=(self.number_of_classes, len(y)))

        for i in range(len(y)):
            Y_binary[y[i]][i] = 1

        for i in range(num_epochs):
            for j in range(0, X_without_1.shape[1], batch_size):

                if j+batch_size <= X_without_1.shape[1]:
                    col = j + batch_size
                else:
                    col = X_without_1.shape[1]

                X_transpose = np.transpose(X_with_1[:, j:col])

                if learning == "Delta":
                    self.weights = self.weights + alpha * np.dot((Y_binary[:, j:col] - self.predict(X_without_1[:, j:col])), X_transpose)

                elif learning == "Filtered":
                    self.weights = (1 - gamma) * self.weights + alpha * np.dot(Y_binary[:, j:col], X_transpose)

                elif learning == "Unsupervised_hebb":
                    self.weights = self.weights + alpha * np.dot(self.predict(X_without_1[:, j:col]), X_transpose)
    ...
    def calculate_percent_error(self, X, y):
        ...
        """
        return np.average(np.where((y - np.argmax(self.predict(X), axis=0)) != 0, 1, 0))
    ...
    def calculate_confusion_matrix(self, X, y):
        ...
        argm = np.argmax(actual, axis=0)

        for i in range(0,len(y)):
            confusion_matrix[y[i]][argm[i]] += 1
...
if __name__ == "__main__":
    ...
    percent_error = []
    for k in range(10):
        model.train(X_train_vectorized, y_train, batch_size=300, num_epochs=2, alpha=0.1, gamma=0.1, learning="Unsupervised_hebb")
