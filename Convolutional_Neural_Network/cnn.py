...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,), name=""):
        """
         ...
         :return: None
         """
        inp = keras.Input(shape=shape, name=name)
        self.model = keras.Model(inputs=inp, outputs=inp)
    ...
    def append_dense_layer(self, num_nodes, activation="relu", name="", trainable=True):
        """
         ...
         :return: None
         """
        dense = keras.layers.Dense(num_nodes, activation=activation, name=name, trainable=trainable)(self.model.output)
        self.model = keras.Model(self.model.input, dense)
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         :return: Layer object
         """
        conv = keras.layers.Conv2D(num_of_filters, kernel_size, strides=strides, padding=padding, activation=activation, name=name, trainable=trainable)(self.model.output)
        self.model = keras.Model(self.model.input, conv)
        return conv
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2,name=""):
        """
         ...
         :return: Layer object
         """
        pooling = keras.layers.MaxPooling2D(pool_size=pool_size, padding=padding, strides=strides, name=name)(self.model.output)
        self.model = keras.Model(self.model.input, pooling)
        return pooling
    ...
    def append_flatten_layer(self, name=""):
        """
         ...
         :return: Layer object
         """
        flatten = keras.layers.Flatten(name=name)(self.model.output)
        self.model = keras.Model(self.model.input, flatten)
        return flatten
    ...
    def get_weights_without_biases(self, layer_number=None, layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        return None if len(self.model.get_layer(name=layer_name, index=layer_number).weights) is 0 else self.model.get_layer(name=layer_name, index=layer_number).weights[0].numpy()
    ...
    def get_biases(self, layer_number=None, layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        return None if len(self.model.get_layer(name=layer_name, index=layer_number).weights) is 0 else self.model.get_layer(name=layer_name, index=layer_number).weights[1].numpy()
    ...
    def predict(self, X):
        ...
        """
        X_hat = X
        return self.model.predict(X_hat)
    ...
    def train(self, X_train, y_train, batch_size, num_epochs):
        ...
        return self.model.fit(X_train, y_train,batch_size,num_epochs,steps_per_epoch=None,validation_steps=None)
