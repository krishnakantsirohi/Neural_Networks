...
class MultiNN(object):
    ...
    def predict(self, X):
        ...
        for i in range(0, len(self.activations)):
            net_value = tf.add(tf.matmul(X_hat, self.weights[i]), self.biases[i])
            pred = self.activations[i](net_value)
            X_hat = pred
        return pred
        ...
        for _ in range(num_epochs):
            for step, (x, y) in enumerate(dataset):
                with tf.GradientTape(persistent=True) as tape:
                    loss = self.cross_entropy_loss(y, self.predict(x))
                    for i in range(0, len(self.weights)):
                        print(loss)
                        dloss_dweights, dloss_dbiases = tape.gradient(loss, [self.weights[i], self.biases[i]])
                        self.weights[i].assign_sub(alpha * dloss_dweights)
                        self.biases[i].assign_sub(alpha * dloss_dbiases)
    ...
    def calculate_confusion_matrix(self, X, y):
        ...
        """
        return confusion_matrix(y, tf.argmax(self.predict(X), 1))
